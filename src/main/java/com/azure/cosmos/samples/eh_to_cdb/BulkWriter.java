...
class BulkWriter implements AutoCloseable {
    ...
    private static BulkImportFailure createFailure(
        ...
        failure.setDocumentIdsFailedToImport(
            Collections.singletonList(itemOperation.<OperationContext>getContext().getId()));
        ...
        try {
            failure.setDocumentsFailedToImport(
                Collections.singletonList(Configs.mapper.writeValueAsString(doc))
            );
        } catch (JsonProcessingException e) {
            failure.setDocumentsFailedToImport(Collections.singletonList(e.toString()));
        }
